...
rec {
  ...
  commandToPackage =
    cmd:
    if cmd.name != devshellMenuCommandName && cmd.command == null && cmd.package == null then
      null
    else
      assert lib.assertMsg (cmd.command == null || cmd.name != cmd.command)
        "${commandsMessage} in ${
          lib.generators.toPretty { } cmd
        }, ${toString cmd.name} cannot be set to both the `name` and the `command` attributes. Did you mean to use the `package` attribute?";
      assert lib.assertMsg
        (
          (cmd.package != null && cmd.command == null)
          || (cmd.command != null && cmd.command != "" && cmd.package == null)
        )
        "${commandsMessage} ${
          lib.generators.toPretty { } cmd
        } expected either a non-empty command or a package attribute, not both.";
    ...
    let
      cleanName =
        ...
        then
          cmd
          // {
            name = "${if cmd.prefix != null then cmd.prefix else ""}${
          ...
          ) "${commandsMessage} some command is missing a `name`, a `prefix`, and a `package` attributes.";
          let
            name = lib.pipe cmd [
              resolveName
      ...
      commandLengths = map ({ name, ... }: lib.stringLength name) commands;

      maxCommandLength = lib.foldl' (max: v: if v > max then v else max) 0 commandLengths;

      commandCategories = lib.unique (
        (lib.zipAttrsWithNames [ "category" ] (_: vs: vs) commands).category
      );

      commandByCategoriesSorted = lib.attrValues (
        lib.genAttrs commandCategories (
          category:
          lib.nameValuePair category (
            lib.sort (a: b: a.name < b.name) (lib.filter (x: x.category == category) commands)
          )
        )
      );
        ...
        let
          ...
          opCmd =
            ...
            let
              ...
              processHelp =
                x:
                if (if interpolate != null then interpolate else menuConfig.interpolate) then
                  ''\''
                  + "\n"
                  + ''
                    "$(
              ...
              command = ''
                ${highlyUnlikelyName}=${if help == null || help == "" then "" else processHelp help}
